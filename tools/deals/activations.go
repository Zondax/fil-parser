	...
	if version.NodeVersion() < tools.V3.NodeVersion() {
		...
		for _, dealID := range dealIDs {
			dealSpaceInfo = append(dealSpaceInfo, &types.DealsSpaceInfo{
				...
				NonVerifiedDealSpace: nonVerifiedDealWeight.Uint64(),
				VerifiedDealSpace:    verifiedDealWeight.Uint64(),
	...
	parseDeals := func(params, ret map[string]interface{}) error {
		...
		for _, dealID := range dealIDs {
			dealSpaceInfo = append(dealSpaceInfo, &types.DealsSpaceInfo{
				...
				TxCid:                tx.TxCid,
				GroupDealIDs:         dealIDs,
				NonVerifiedDealSpace: nonVerifiedDealSpace.Uint64(),
				VerifiedDealSpace:    verifiedDealSpace.Uint64(),
				ActionType:           tx.TxType,
				TxTimestamp:          tx.TxTimestamp,
			})
		}
