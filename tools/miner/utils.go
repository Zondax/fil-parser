...
func (eg *eventGenerator) consolidateIDAddress(idAddress uint64) (string, error) {
	...
	consolidatedIDAddress, err := actors.ConsolidateToRobustAddress(eg.nodes, addr, eg.helper, eg.logger, eg.config.RobustAddressBestEffort, true)
	if err != nil {
...
func (eg *eventGenerator) consolidateAddress(addrStr string) (string, error) {
	...
	consolidatedAddress, err := actors.ConsolidateToRobustAddress(eg.nodes, addr, eg.helper, eg.logger, eg.config.RobustAddressBestEffort, true)
	if err != nil {
