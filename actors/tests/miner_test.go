...
func TestActorParserV2_MinerWithParamsOrReturn(t *testing.T) {
	...
	actor, err := p.GetActor(manifest.MinerKey, &metrics.ActorsMetricsClient{MetricsClient: metrics2.NewNoopMetricsClient()})
	...
	for _, tt := range minerWithParamsOrReturnTests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, _, err := actor.Parse(context.Background(), network, tools.V18.Height(), tt.txType, msg, msgRct, cid.Undef, filTypes.EmptyTSK)
			require.NoError(t, err)
			require.NotNil(t, got)
			require.Contains(t, got, tt.key, fmt.Sprintf("%s could no be found in metadata", tt.key))
			require.NotNil(t, got[tt.key])
