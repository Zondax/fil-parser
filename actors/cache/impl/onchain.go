...
func (m *OnChain) NewImpl(source common.DataSource) error {
	// Node datastore is required
	if source.Node == nil {
		zap.S().Panic("[ActorsCache] - Node ptr is nil")
	}
...
func (m *OnChain) retrieveActorFromLotus(add address.Address, key filTypes.TipSetKey) (cid.Cid, error) {
	...
	if err != nil {
		...
		if err != nil {
			zap.S().Errorf("[ActorsCache] - retrieveActorFromLotus: %s", err.Error())
			return cid.Cid{}, err
		}
	}
...
func (m *OnChain) retrieveActorPubKeyFromLotus(add address.Address, reverse bool) (string, error) {
	...
	if err != nil {
		zap.S().Errorf("[ActorsCache] - retrieveActorPubKeyFromLotus: %s", err.Error())
		return "", common.ErrKeyNotFound
	}
